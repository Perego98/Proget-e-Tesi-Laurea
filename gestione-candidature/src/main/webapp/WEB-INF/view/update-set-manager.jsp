...
<div class="panel panel-default">
		...
		<h2>Assegna Manager</h2>
		<p>Supervisore Attuale: ${candidato.supervisore.userName}</p>
		</div>
		<div  class="panel-body">
			...
			<table class="table table-striped">
					...
					<c:forEach var="tempManager" items="${managers}">
							<c:if test="${tempManager.qualified == true}">
							<tr>
							<c:url var="updateLink" value="/hr/processUpdateSupervisoreCandidatoForm">
								<c:param name="codFiscale" value="${candidato.codiceFiscale}" />
								<c:param name="userUsername" value="${tempManager.userName}" />
							</c:url>
							
								<td>${tempManager.userName}</td>
								<td>${tempManager.firstName}</td>
								<td>${tempManager.lastName}</td>
								<td><input type="button" value="Collega"
